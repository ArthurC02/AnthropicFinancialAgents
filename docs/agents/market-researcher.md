...
| 想調的業務內容 | 改哪個檔 | 怎麼改 |
|---|---|---|
| 同業圈選慣例／comps 指標定義（EV/EBITDA、P/E 等） | `vertical-plugins/financial-analysis/skills/comps-analysis/SKILL.md` | 改值 → sync |
| 產業概覽產法／競爭格局結構（市占、護城河指標） | `vertical-plugins/financial-analysis/skills/sector-overview/SKILL.md`、`competitive-analysis/SKILL.md` | 改框架 → sync；臨時可直接 prompt 給 |
| 點子清單篩選規則（量化門檻、主題掃描條件） | `vertical-plugins/financial-analysis/skills/idea-generation/SKILL.md` | 改規則 → sync；門檻臨時可 prompt 給 |
| 簡報範本（品牌版型、每張觀點格式） | `vertical-plugins/financial-analysis/skills/pptx-author/SKILL.md`；或用 `/ppt-template` 產版型 | 改範本 → sync |
| 流程／兩個 stop 點／守則 | `agents/market-researcher.md` 的 Workflow／Guardrails | 直接改 agents 檔 |
| 用哪些 skill／幾個 sub-agent | `agents/market-researcher.md` Skills 行；`cookbooks/market-researcher/agent.yaml` callable_agents | 改 agents 檔或 agent.yaml |
| 接 capiq（mock → 真實系統） | vertical `.mcp.json` 的 `capiq` url | 改 url 指真實 CapIQ／S&P feed；server 名不改 |
| 接 factset（真實資料） | core plugin 已定義，網址照用 | 申請訂閱＋API 金鑰，網址照用 |
...
**接真實系統要做到**（上線必補）
- 🛠️ `capiq`：目前指向本機 mock（`mock-mcp/`，假 CSV）；上線將 vertical `.mcp.json` 的 `capiq` url 改指真實 CapIQ／S&P feed，server 名與 frontmatter `tools:` 不動。
- 🛠️ `factset`：公開供應商，向 FactSet 申請訂閱＋API 金鑰，網址照用。
- 👤 **人工覆核不變**：兩次停下來給人審（comps 後、報告草擬後），分析師核准才發布；CMA 版寫檔權隔離給 note-writer sub-agent。
