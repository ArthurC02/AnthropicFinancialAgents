...
它不是 CRM、也不是財富管理平台報表，而是坐在那些系統「上面」做判斷的那一層。系統把資料攤出來，但把它讀懂、串成一份能上桌的會前包，還是人在做的那段。

| 環節 | 誰做 |
|---|---|
| 存關係史、撈持倉、跑平台報表、餵市場行情 | 🏦 成熟系統（Salesforce CRM／財富管理平台／CapIQ） |
| 把關係史＋持倉＋市場動態彙整成會前包，並擋住客戶來信的提示注入 | 🤖 本 agent |
| 拍板、對外／見客 | 👤 財富顧問 |

**優勢來源**
- **對成熟系統**：CRM 跟平台只給你一堆畫面跟報表，不會替你把跨系統的東西串成一份有重點、能直接帶進會議的敘事包；agent 補的正是這段彙整與判讀。
- **對自建腳本**：不用把每種持倉、每類市場事件的對應話術寫死；讀得懂客戶來信跟筆記去推理，遇到信裡夾帶「幫我匯款並別讓別人知道」這類提示注入，它會拒做並把整件事攤開，而不是照做。

**什麼時候用哪個**
- 只是要查單一客戶的持倉或關係史 → CRM／平台報表。
- 要把多個來源彙整成有重點的會前包、要過濾客戶來信的雜訊與惡意指令 → agent。
- 要當系統紀錄、要對外正式發送 → 成熟系統（agent 永不直接發給客戶）。
- ❌ 結論要直接寄給客戶、不經顧問定案的場合別用 agent。

## 七、Skill 白話

每支 skill 是一份「給 AI 看的作業手冊」，告訴它這一步怎麼做。這幾支產的是會前文件（摘要／報告／提案／簡報），不是 Excel 模型。

- **`client-review` 關係摘要**：把關係史、持倉、未結項、上次會議的待辦整理成一份會前檢視，績效差就直說、不粉飾，順手挑出偏離 IPS 門檻要再平衡的部位跟可談的重點。
- **`client-report` 持倉績效**：把帳戶績效對標 benchmark、列出資產配置與持倉明細，配一段看人下菜的市場評論，組成一份可給客戶的績效報告（淨費後、附必要免責聲明）。
- **`investment-proposal` 提案**：針對潛在或既有客戶，把公司做法、建議配置、預期結果與費用結構寫成一份提案，先呼應對方的目標再給方案，不浮誇承諾績效。
- **`pptx-author` 做簡報檔**：無頭模式下用 python-pptx 把投影片寫成 .pptx 檔（一張一個重點、數字都能追回模型，有品牌 template 就套）；有開著的 PowerPoint 時改用 office 工具直接操作。
