...
整個過程四步。**第 0 步是關鍵**——不先做會直接報錯（錯誤訊息見下方）。
...
☐ 第①步有沒有圈出 8–15 家代表公司？
☐ 第④步「同業比較」的數字——沒接資料源時，應該掛 [UNSOURCED]
...
沒接資料源、又沒跑本機 mock 時，agent 拿不到精準財報倍數，照它的規矩會把整欄標起來，而不是瞎編。實際輸出大致像這樣：

```
④ 同業比較(comps)　← 這就是「沒資料源」的破口
  公司            市值        EV/EBITDA     營收成長
  Vertiv (VRT)    [UNSOURCED]  [UNSOURCED]   [UNSOURCED]
  nVent (NVT)     [UNSOURCED]  [UNSOURCED]   [UNSOURCED]
```
> 🔴 web-only 模式拿不到精準財報倍數，所以整欄掛 `[UNSOURCED]`——這是**正常且正確**的行為（它遵守 [market-researcher.md](agents/market-researcher.md) 裡「查不到就標、不瞎掰」的規矩）。
> 這一欄，就是你接資料源（跑本機 mock 或接真實廠商）要補實的缺口。
...
階段 A 跑順、建立信任後，再來把第④步那欄 `[UNSOURCED]` 換成有來源的數字。

```
┌─ 升級步驟 ────────────────────────────────────────────────┐
│ ① 想離線就先有 comps：跑 repo 內建的本機 mock              │
│    python3 mock-mcp/run_all_http.py                        │
│    → capiq 會回假資料填上 comps（僅供 demo，數字是假的）   │
│                                                            │
│ ② 要真實數字：factset 在 .mcp.json 已填好廠商網址         │
│    → 去 FactSet 申請 API 金鑰即可                          │
│ ③ capiq 預設指本機 mock → 上線把它的 url 改指你的         │
│    CapIQ／S&P 來源（在 financial-analysis/.mcp.json 換掉） │
│ ④ 客製同業比較的指標定義／同業圈選慣例                    │
...
> 📌 接上資料源後：第④步的市值、EV/EBITDA、成長率、毛利率就有數字了——跑本機 mock 是假資料（供 demo），接真實廠商才是官方資料，`[UNSOURCED]` 隨之消失。
